...
#identify the files you want to read in 
  ...
  NH3_files <- c("Raw Data/20260608_2M_Phragrr_May_Nh4.csv",
                 "Raw Data/20260608_Phrag_May_NH4.csv")
...
# Define the file path for QAQC log file - NO Need to change just check year 
  ...
  final_path <- "Processed Data/Phrag_Porewater_NH4_202605.csv"

#record any notes about the run or anything other info here (Leave empty otherwise): 
  run_notes <- "Two Separate runs done on 20260608, one PE check passes one fails"
...
samples <- df_all_cor %>%  
  filter(str_detect(Sample_Name, c("P"))) %>%
  filter(!str_detect(Sample_Name, c("PE")))
...
samples_flagged1 <- samples %>% mutate(Dilution = ifelse(Dilution == 0, 1, Dilution )) %>%
  ...
  filter(Conc_flag == "Within_Range" | n() == 1) %>% ungroup()
...
viz_NH3_plot_2 <- ggplot(data = NH3_forplot, aes(x = NH3_Conc_uM, y = as.numeric(Depth_cm), color = Plot_Color)) +
  geom_point(size = 3) +
  geom_path(linewidth = 1) +
  scale_color_manual(
    values = c(
