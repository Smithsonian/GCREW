---
title: "CO2xNxPhrag: Porewater SO4/Cl"
...
lapply(c(
  ...
  "matrixStats", "gridExtra", "grid", "tidyverse", "knitr", "ggpattern"), 
...
###### Run information - PLEASE CHANGE
  Date_Run = "2026-09-14"  #Date that instrument was run
...
#file path and name for raw summary data file 
  raw_file_name_cl = "Raw Data/CO2xNxPhrag_202607_Cl.txt"
  raw_file_name_so4 = "Raw Data/CO2xNxPhrag_202607_SO4.txt"

#file path and name of processed data file 
  processed_file_name = "Processed Data/GCReW_CO2xNxPhrag_PW_Processed_Cl_SO4_202607.csv" 
...
#downloaded metadata csv - downloaded from Google drive as csv for this year
  ...
  Log_path = "Raw Data/COMPASS_Synoptic_Cl_SO4_QAQClog_2026.csv"
...
dionex_metadata <- raw_metadata %>%
  filter(grepl(Project_name, Project, ignore.case = TRUE)) %>%
...
#create a new dataframe and remove spks from sample dataframe 
dat_spks <- all_dat %>%  
...
#change sample IDs to match metadata
all_data_flagged <- all_data_flagged %>%
  mutate(sample_ID = paste0(sample_ID, "_", sample_month, "_", sample_year))
...
## Visualize Data
```{r Visualize Cl Data, echo=FALSE}
cat("Visualize Data")
...
Cl_forplot <- Cl_forplot[order(Cl_forplot$Depth_cm), ]

viz_Cl_plot_v2 <- ggplot(
  data = Cl_forplot,
  aes(
  ...
  labs(
    x = " ",
    y = "Cl (mM)",
    title = "CO2xNxPhrag: Porewater Chloride",
    fill = "CO2 Treatment:",
    color = "N Treatment:"
  ) +
...
viz_Cl_plot_2 <- ggplot(data = Cl_forplot, aes(x = Cl_Conc_mM, y = as.numeric(Depth_cm), color = Plot_Color, shape = Sub_Plot)) +
  geom_point(size = 3) +
  ...
  labs(
    x = "Cl (mM)",
    y = "Depth (cm)",
    title = "CO2xNxPhrag: Porewater Cl by Replicate",
    color = "Replicate"
  ) +
...
SO4_forplot <- SO4_forplot[order(SO4_forplot$Depth_cm), ]

viz_SO4_plot_v2 <- ggplot(
  data = SO4_forplot,
  aes(
    x = factor(as.factor(CO2_Treatment), levels = c("eCO2", "aCO2")),
    y = SO4_Conc_mM,
  ...
  labs(
    x = " ",
    y = "SO4 (mM)",
    title = "CO2xNxPhrag: Porewater Sulfate",
    fill = "CO2 Treatment:",
    color = "N Treatment:"
  ) +
  theme(legend.position = "right") +
...
viz_SO4_plot_2 <- ggplot(data = SO4_forplot, aes(x = SO4_Conc_mM, y = as.numeric(Depth_cm), color = Plot_Color, shape = Sub_Plot)) +
  geom_point(size = 3) +
  geom_path(linewidth = 1) +
  scale_color_manual(
    values = c(
  ...
  labs(
    x = "SO4 (mM)",
    y = "Depth (cm)",
    title = "CO2xNxPhrag: Porewater SO4 by Replicate",
    color = "Replicate"
  ) +
  scale_y_reverse(breaks = c(20, 40), limits = c(45, 15)) +
  theme_classic() +
...
final_data <- final_data %>%
  select(
         ...
         Community, Nickname, Plot, SubPlot, Depth_cm, CO2_Treatment, Temp_Treatment, N_Treatment, 
