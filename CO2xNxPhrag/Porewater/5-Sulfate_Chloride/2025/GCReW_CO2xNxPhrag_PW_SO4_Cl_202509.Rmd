...
###### Run information - PLEASE CHANGE
  Date_Run = "2026-01-22"  #Date that instrument was run
...
#file path and name for raw summary data file 
  raw_file_name_cl = "Raw Data/CO2xNxPhrag_202509_Cl.txt"
  raw_file_name_so4 = "Raw Data/CO2xNxPhrag_202509_SO4.txt"

#file path and name of processed data file 
  processed_file_name = "Processed Data/GCReW_CO2xNxPhrag_PW_Processed_Cl_SO4_202509.csv" 
...
data_plotting$CO2_Treatment = factor(data_plotting$CO2_Treatment, levels = c("Ambient", "Elevated"))
data_plotting <- data_plotting[order(data_plotting$CO2_Treatment), ]
# 
# data_plotting$Community = factor(data_plotting$Community, levels = c("SCAM", "SPPA"))
# data_plotting <- data_plotting[order(data_plotting$Community), ]

data_plotting$Depth_cm = factor(data_plotting$Depth_cm, levels = c("20", "40", "80"))
data_plotting <- data_plotting[order(data_plotting$Depth_cm), ]

#Add cm units for depths
depth_labels <- c(
  "20" = "20 cm",
  "40" = "40 cm", 
  "80" = "80 cm"
...
viz_cl_plot <- ggplot(data_plotting, aes(x = as.factor(CO2_Treatment), y = Cl_ppm, fill = as.factor(CO2_Treatment))) +
  geom_bar(stat = "identity", position = position_dodge2(preserve = "single"), color="black") +   
  facet_grid(~ Depth_cm, labeller = labeller(Depth_cm = depth_labels), scales="free_x") +
  scale_fill_manual(values = c(
    "Elevated" = "red", 
    "Ambient" = "blue"
  )) +
  theme_classic() +
  labs(x = " ", y = "Cl (mg/L)", title = "Samples: Chloride", fill="CO2_Treatment") +
...
viz_so4_plot <-  ggplot(data_plotting, aes(x = as.factor(CO2_Treatment), y = SO4_ppm, fill = as.factor(CO2_Treatment))) +
  geom_bar(stat = "identity", position = position_dodge2(preserve = "single"), color="black") +   
  facet_grid(~ Depth_cm, labeller = labeller(Depth_cm = depth_labels), scales="free_x") +
  scale_fill_manual(values = c(
    "Elevated" = "red", 
  ...
  theme_classic() +
  labs(x = " ", y = "SO4 (mg/L)", title = "Samples: SO4", fill="CO2_Treatment") +
  scale_x_discrete(drop = TRUE) + 
  theme(legend.position = "right") +
  theme(axis.text.x = element_text(angle = 0, hjust = 0.5), 
  panel.border = element_rect(color = "black", fill = NA, linewidth = 0.8))
