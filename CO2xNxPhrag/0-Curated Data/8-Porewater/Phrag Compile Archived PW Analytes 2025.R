...
wd <- "S:/Biogeochemistry/GCREW/3-PhragmitesxCO2xN Experiment/0-Curated Data/8-Porewater"
...
sulfide <- read.csv("Phrag Master Porewater Sulfide 2025 (published 02-02-2026).csv")
sulfide$Day_sulfide <- sulfide$Day
sulfide$Day <- NULL
sulfide <- sulfide %>%
  mutate(Month = factor(Month, levels = month.name)) 
sulfide <- sulfide[with(sulfide, order(Year, Month, Chamber, Well, Depth)),]
sulfide <- sulfide %>% 
...
pH <- read.csv("Phrag Master Porewater pH 2025 (published 01-27-2026).csv")
pH$Day_pH <- pH$Day
pH$Day <- NULL
pH <- pH %>%
  mutate(Month = factor(Month, levels = month.name)) 
pH <- pH[with(pH, order(Year, Month, Chamber, Well, Depth)),]
pH <- pH %>% 
  group_by(Year, Month, SampleID) %>% 
  dplyr::mutate(dupe = n()>1)
...
CH4 <- read.csv("Phrag Master Porewater Methane 2025 (published 01-27-2026).csv")
CH4$Day_CH4 <- CH4$Day
CH4$Day <- NULL
CH4 <- CH4 %>%
  mutate(Month = factor(Month, levels = month.name)) 
CH4 <- CH4[with(CH4, order(Year, Month, Chamber, Well, Depth)),]
CH4 <- CH4 %>% 
  group_by(Year, Month, SampleID) %>% 
  dplyr::mutate(dupe = n()>1)
...
NH4 <- read.csv("Phrag Master Porewater NH4 2025 (published 02-13-2026).csv")
NH4$X <- NULL
NH4 <- NH4 %>%
  mutate(Month = factor(Month, levels = month.name)) 
NH4 <- NH4[with(NH4, order(Year, Month, Chamber, Well, Depth)),]
NH4 <- NH4 %>% 
  group_by(Year, Month, SampleID) %>% 
  dplyr::mutate(dupe = n()>1)
...
SO4.Cl <- read.csv("Phrag Master Porewater SO4 & Cl 2025 (published 02-13-2026).csv")
SO4.Cl$Day_SO4.Cl <- SO4.Cl$Day
SO4.Cl$Day <- NULL
SO4.Cl <- SO4.Cl %>%
  mutate(Month = factor(Month, levels = month.name)) 
SO4.Cl <- SO4.Cl[with(SO4.Cl, order(Year, Month, Chamber, Well, Depth)),]
SO4.Cl <- SO4.Cl %>% 
  group_by(Year, Month, SampleID) %>% 
  dplyr::mutate(dupe = n()>1)
...
# Merge Analytes ---------------------------------------------------------------
# Add pH 
all <- full_join(sulfide, pH, by=c('Year','Month', 'Chamber', 'Well', "Depth", 'CO2', 'Nitrogen', 'Treatment','SampleID'))

# Add CH4 
all <- full_join(all, CH4, by=c('Year','Month', 'Chamber', 'Well', "Depth", 'CO2', 'Nitrogen', 'Treatment','SampleID'))

# Add NH4
all <- full_join(all, NH4, by=c('Year','Month', 'Chamber', 'Well', "Depth", 'CO2', 'Nitrogen', 'Treatment','SampleID'))

# Add SO4 and Cl
all <- full_join(all, SO4.Cl, by=c('Year','Month', 'Chamber', 'Well', "Depth", 'CO2', 'Nitrogen', 'Treatment','SampleID'))
...
all <- all[with(all, order(Year, Month, Chamber, Well, Depth)),]
...
write.csv(all, "Phrag Master Porewater All Data 2011-2025 (published 02-13-2026).csv", row.names = F)
...
# setwd(paste0(wd,"/0-All Porewater Data", sep=""))
# write.csv(NA_all, "Phrag Percent Missing Data 2011-2023.csv", row.names = F)
# write.csv(NA_100_select, "Phrag Months and Years with All Data Missing 2011-2023.csv", row.names = F)
# 