...
write_monthly_yearly_waterlevel <- function(dt, L0_NormalizedData_dir) {
  ...
  for (y in year){
    ...
    #file_yearly <- list.files(file_path, pattern = c(table, y), recursive = T, full.names = T, ignore.case = T)
    ...
    if(!file.exists(file_path)){
      ...
      if (!dir.exists(paste0(rawCSVDataArchive_dir))){
        dir.create(paste0(rawCSVDataArchive_dir), recursive = TRUE)
      }
