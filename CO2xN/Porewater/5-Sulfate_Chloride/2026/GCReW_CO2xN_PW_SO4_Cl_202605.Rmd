...
###### Run information - PLEASE CHANGE
  Date_Run = "2026-07-07"  #Date that instrument was run
  ...
  Site = "GCReW"
  Project_name = "CO2xN"
  run_notes = "One bad DUP from Cl, two bad DUPS from SO4. 
One bad SPK. Standard curve looks mostly good but the first standards are still low (despite remaking them). 
We might need to re-purchase the standards.
  " #any notes from the run

###### File Names - PLEASE CHANGE 
#file path and name for raw summary data file 
  raw_file_name_cl = "Raw Data/CO2xN_202605_Cl.txt"
  raw_file_name_so4 = "Raw Data/CO2xN_202605_SO4.txt"

#file path and name of processed data file 
  processed_file_name = "Processed Data/GCReW_CO2xN_PW_Processed_Cl_SO4_202605.csv" 
...
final_data <- final_data %>%
  select(
         ...
         Community, Nickname, Plot, Depth_cm, CO2_Treatment, N_Treatment, 
...
## Visualize Data SO4
```{r Visualize Data SO4, echo=FALSE}
...
viz_SO4_plot2 <- ggplot(data = SO4_forplot, aes(
...
#make plot
viz_SO4_plot_2 <- ggplot(data = SO4_forplot, aes(x = SO4_Conc_mM, y = as.numeric(Depth_cm), color = Plot_Color)) +
  geom_point(size = 3) +
  ...
  labs(
    x = "SO4 (mM)",
    y = "Depth (cm)",
    title = "CO2xN: Porewater SO4 by Replicate",
    color = "Replicate"
  ) +
...
## Visualize Data Cl
```{r Visualize Data Cl, echo=FALSE}
...
viz_Cl_plot2 <- ggplot(data = Cl_forplot, aes(
  x = factor(as.factor(CO2_Treatment), levels = c("eCO2", "aCO2")),
  y = Cl_Conc_mM,
  fill = as.factor(Treatment),
  color = as.factor(Treatment)
...
#make plot
viz_Cl_plot_2 <- ggplot(data = Cl_forplot, aes(x = Cl_Conc_mM, y = as.numeric(Depth_cm), color = Plot_Color)) +
  geom_point(size = 3) +
  geom_path(linewidth = 1) +
  scale_color_manual(
    values = c(
  ...
  labs(
    x = "Cl (mM)",
    y = "Depth (cm)",
    title = "CO2xN: Porewater Cl by Replicate",
    color = "Replicate"
  ) +
  scale_y_reverse(breaks = c(20, 40, 80), limits = c(90, 10)) +
  theme_classic() +
