...
Slope_H2S_raw_list <- map(split_data, ~ .x %>% mutate(Curve = "H2S",
 ...
 R2_Check_NRMVBL <- R2_Check_diff %>% filter(RMVBL != "Yes") %>% mutate(STD_ABS_rm = Mean_Abs)
 ...
 R2_Check_bind <- rbind(R2_Check_na,R2_Check_NRMVBL) %>% na.omit()
 
 R2_Check_bind <- R2_Check_bind[order(R2_Check_bind$Plate,R2_Check_bind$IDs), ]
 
 R2_Check_fin <- R2_Check_bind %>% group_by(Plate) %>% mutate(N_stds = length(STD_ABS_rm)) %>% group_by(Plate, CV_Flag) %>%
 ...
 split_data_rm <- R2_Check_fin %>% dplyr::select(Plate, IDs, Conc, STD_ABS_rm, CV_Flag, R2_Flag) %>%
      distinct() %>%
      group_by(Plate) %>%
      group_split()
 
 Slope_H2S_raw_list_rm <- map(split_data_rm, ~ .x %>% mutate(Curve = "H2S",
                                        R2 = summary(lm(Conc ~ STD_ABS_rm, data = .))$adj.r.squared , 
                                        Slope = coef(lm(Conc ~ STD_ABS_rm, data = .))[2],
                                        Intercept = coef(lm(Conc ~ STD_ABS_rm, data = .))[1]))

 Slope_H2S_raw_rm <- bind_rows(Slope_H2S_raw_list_rm)
  
 Slope_H2S_Std_Crvs_rm <- Slope_H2S_raw_rm %>% dplyr::select(Plate, Curve, R2, Slope, Intercept) %>% distinct() %>% mutate(R2_Flag = ifelse(R2 >= r2_cutoff, "R2_Good", "R2_Bad"))
