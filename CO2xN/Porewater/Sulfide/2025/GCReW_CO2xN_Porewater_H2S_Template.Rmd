...
H2S_STD_CV_fin <- H2S_STD_cv_new %>% dplyr::select(Plate, Wells, Abs, Removed, IDs, Dilution, Conc, Mean_Abs2, CV_Flag2) %>% mutate(Mean_Abs = Mean_Abs2, CV_Flag = CV_Flag2) %>% dplyr::select(Plate, Wells, Abs, Removed, IDs, Dilution, Conc, Mean_Abs, CV_Flag)
...
Slope_H2S_raw_list <- map(split_data, ~ .x %>% mutate(Curve = "H2S",
 ...
 Slope_H2S_Std_Crvs <- Slope_H2S_raw %>% dplyr::select(Plate, Curve, R2, Slope, Intercept) %>% distinct() %>%
