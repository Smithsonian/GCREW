...
#Calculate CVs
H2S_STD_cv <- H2S_stds %>% group_by(Plate, IDs) %>% mutate(Mean_Abs = mean(Abs)) %>% mutate(Abs_SD = sd(Abs)) %>% mutate(CV = (Abs_SD /
                                                                                                                                 Mean_Abs) * 100) %>% mutate(CV_Flag = ifelse(CV >= cv_flag_stds, "CV_High", "CV_Good")) %>% ungroup()
...
CH4_Curve <- ggplot(Slope_H2S_raw_rm, aes(STD_ABS_rmv, Conc, color = Plate)) + geom_point(size =
                                                                          4) +
...
new_log_stds <- as.data.frame(new_log_stds)



log <- read.csv(log_path)
log <- log %>% mutate(Run_Date = as.Date(Run_Date, format = "%m/%d/%Y"))
head(log)
log <- log[, -c(1)]
log <- rbind(log, new_log_stds)

unique_log <- log %>% distinct(.keep_all = TRUE)

Slopes_chk <- ggplot(unique_log, aes(x = Run_Date, y = Slope, col = Plate)) +
  geom_point(size = 4) +
  theme_bw()
