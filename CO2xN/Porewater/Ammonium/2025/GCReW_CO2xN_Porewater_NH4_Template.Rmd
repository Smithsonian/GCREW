...
# Define the file path for QAQC log file - NO Need to change just check year 
  file_path <- here("..", "GCReW", "GCReW_SEAL_StdCrv_QAQC_Log.csv")
...
Spike_Log_Plot <- ggplot(Joined_QAQC, aes(x = Run_Date, y = Spike_CV, col = Run)) +
  geom_point(size = 2) +
...
TopStd_Log_Plot <- ggplot(Joined_QAQC, aes(x = Run_Date, y = Top_Std_Absorbance, col = Run)) +
  geom_point(size = 2) +
  geom_path(linewidth = 1.5) +
  facet_grid(~Test, scales = "free") +
  theme_classic()
...
PE_Log_Plot <- ggplot(Joined_QAQC, aes(x = Run_Date, y = Mean_PE_Conc, col = Run)) +
  geom_point(size = 2) +
  geom_path(linewidth = 1.5) +
  facet_grid(~Test, scales = "free") +
  theme_classic()
