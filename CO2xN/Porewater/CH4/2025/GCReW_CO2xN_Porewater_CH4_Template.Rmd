...
volume = 0.012 #volume of gas (in liters) in syringe
...
#Calculate CH4 concentrations in ppm
Samples$CH4_Conc_umol <- (Samples$CH4_Conc_ppm_dilcorr * ((1 * volume /
                                                             1000) / (R * (TempC + CTK)))) / (volume / 1000)
...
ch4_samples1 <-  ggplot(data = Samples, aes(x = Sample_ID, y = CH4_Conc_umol)) +
  ...
  theme_classic() +
  labs(x = " ", y = "CH4 (umol)", title = "CH4 Gas in Water") +
  theme(axis.text.x = element_text(
    angle = 90,
    vjust = 0.5,
    hjust = 1
...
Samples1 <- df_all_clean[, c(7, 9:14, 16:23, 25)]
...
#order columns
org_fin_data <- final_data_labeled[, c(15, 7, 13, 12, 8, 9, 10, 11, 4, 5, 1, 2, 3, 6, 14, 17, 16)]

#rename columns
org_labeled_fin_data <- org_fin_data %>%
  rename(
    Project = Experiment,
    Plot = Plot_Name,
