...
if(summary(high_CH4_lm)$adj.r.squared < r2_cutoff) {message("High_R2 is Bad")} else {message("High R2 Good") }

if(summary(low_CH4_lm)$adj.r.squared < r2_cutoff) {message("Low_R2 is Bad")} else {message("Low R2 Good")}

#create R2 flags

flags <- ifelse(summary(high_CH4_lm)$adj.r.squared < r2_cutoff, paste0(flags, "High_R2 is Bad"), flags)

flags <- ifelse(summary(low_CH4_lm)$adj.r.squared < r2_cutoff, paste0(flags, "; Low_R2 is Bad"), flags)
```
...
if(CH4_chks_percent >= chks_flag) {message(">60% of CH4 Check Standards are within range of expected concentration - PROCEED")} else {message("<60% of CH$ Check Standards are within range of expected concentration - REASSESS")}
...
flags <- ifelse(CH4_chks_percent >= chks_flag, flags, paste0(flags, "; Check Standards out Of Range"))
...
#report out if flags indicate need for rerun
if(CH4_blks_percent >= blks_flag) {message(">60% of CH4 Blanks are below the lower 25% quartile of samples - PROCEED")} else {message("<60% of CH4 blaks are lower 25% quartile of samples - REASSESS")}
...
flags <-  ifelse(CH4_blks_percent >= blks_flag, flags, paste0(flags, '; Blanks out of Range'))
...
viz_CH4_plot2 <- ggplot(data = CH4_forplot, aes(x = as.factor(CO2_Treatment), y = CH4_Conc_umol, fill=as.factor(CO2_Treatment))) +
       geom_bar(stat = "identity", position = position_dodge2(preserve = "single"), 
                 color="black") + 
