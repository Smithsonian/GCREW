...
wd <- "S:/Biogeochemistry/GCREW/2-CO2xN Experiment/0-Curated Data/8-Porewater"
...
sulfide <- read.csv("CO2xN Master Porewater Sulfide 2025 (published 01-16-2026).csv")
...
pH <- read.csv("CO2xN Master Porewater pH 2025 (published 01-16-2026).csv")
pH$Day_pH <- pH$Day
pH$Day <- NULL
pH <- pH %>%
  mutate(Month = factor(Month, levels = month.name)) 
...
CH4 <- read.csv("CO2xN Master Porewater Methane 2025 (published 01-26-2026).csv")
CH4$Day_CH4 <- CH4$Day
CH4$Day <- NULL
CH4 <- CH4 %>%
  mutate(Month = factor(Month, levels = month.name)) 
...
NH4 <- read.csv("CO2xN Master Porewater NH4 2005-2025 (published 01-29-2026).csv")
NH4$X <- NULL
NH4 <- NH4 %>%
  mutate(Month = factor(Month, levels = month.name)) 
...
SO4.Cl <- read.csv("CO2xN Master Porewater SO4 & Cl 2025 (published 01-26-2026).csv")
...
# Merge Analytes ---------------------------------------------------------------
# Add pH 
all <- full_join(sulfide, pH, by=c('Year','Month', 'Chamber', "Depth", 'CO2', 'Nitrogen', 'Treatment','SampleID'))

# Add CH4 
all <- full_join(all, CH4, by=c('Year','Month', 'Chamber', "Depth", 'CO2', 'Nitrogen', 'Treatment','SampleID'))

# Add NH4
all <- full_join(all, NH4, by=c('Year','Month', 'Chamber', "Depth", 'CO2', 'Nitrogen', 'Treatment','SampleID'))

# Add SO4 and Cl
all <- full_join(all, SO4.Cl, by=c('Year','Month', 'Chamber', "Depth", 'CO2', 'Nitrogen', 'Treatment','SampleID'))
...
all[sapply(all, is.na)] <- -99

##Sort by year, month, chamber, and depth 
all <- all %>%
  mutate(Month = factor(Month, levels = month.name)) 

all <- all[with(all, order(Year, Month, Chamber, Depth)),]

all

all <- subset(all, select = c(Year, Month, Sample_Date, SampleID, Chamber, Depth, CO2, Nitrogen, Treatment, H2S, pH, CH4, NH4, SO4, Cl, Salinity))


# Export Data ------------------------------------------------------------------
setwd(paste0(wd,"/0-All Porewater Data", sep=""))
write.csv(all, "CO2xN Master Porewater All Data 2005-2025 (published 01-29-2026).csv", row.names = F)
...
# setwd(paste0(wd,"/0-All Porewater Data", sep=""))
# write.csv(NA_all, "CO2xN Percent Missing Data 2005-2023.csv", row.names = F)
# write.csv(NA_100_select, "CO2xN Months and Years with All Data Missing 2005-2023.csv", row.names = F)
# 
# 
