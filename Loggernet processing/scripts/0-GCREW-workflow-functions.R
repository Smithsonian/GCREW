...
dir_names <- function(mid_path){
  ...
  raw <- file.path(mid_path, "0_raw")
  processed <- file.path(mid_path, "1_processed")
  sorted <- file.path(mid_path, "2_sorted")
  normal <- file.path(mid_path, "3_normal")
  monthly <- file.path(mid_path, "4_monthly_working")
  
  # Create subfolders for working and archive within each step
  raw_work <- file.path(raw, "raw_working")
  raw_archive <- file.path(raw, "raw_archive")
  
  proc_work <- file.path(processed, "processed_working")
  proc_archive <- file.path(processed, "processed_archive")
  
  sort_work <- file.path(sorted, "sorted_working")
  sort_archive <- file.path(sorted, "sorted_archive")
  
  normal_work <- file.path(normal, "normal_working")
  normal_archive <- file.path(normal, "normal_archive")
...
file_import <- function (source_dir, raw_dir, pattern){
  raw_working <- file.path(raw_dir, "raw_working")
  raw_archive <- file.path(raw_dir, "raw_archive")
  # List all files from the source directory folder, does NOT recurse into other folders in directory
  files <- list.files(source_dir, pattern = pattern, all.files = FALSE,
  ...
  file.copy(keep_files, raw_working, overwrite = T)
  print(paste0(length(keep_files), " files have been imported."))
}

## ========================== Function 4: process to CSV =================================================
...
process <- function(raw_dir, proc_dir) { 
  raw_working <- file.path(raw_dir, "raw_working")
  raw_archive <- file.path(raw_dir, "raw_archive")
  proc_working <- file.path(proc_dir, "processed_working")
  ...
  for (n in 1:length(files)) {
    ...
    file.copy(files[n], raw_archive, overwrite = T)
...
sorter <- function(proc_dir, sort_dir){
  proc_working <- file.path(proc_dir, "processed_working")
  proc_archive <- file.path(proc_dir, "processed_archive")
  sort_working <- file.path(sort_dir, "sorted_working")
  sort_archive <- file.path(sort_dir, "sorted_archive")
  ...
  for(n in 1:length(folder_names)){
    ...
    file.copy(abs_files[n], file.path(sort_working, folder_names[n]), overwrite = T)
    # Also copy files to processed_archive once they've been sorted
    file.copy(abs_files[n], proc_archive, overwrite = T)
...
norm_files <- function(sort_dir, logger, table) {
  ...
  sorted_files <- list.dirs(file.path(sort_dir, "sorted_working"), full.names = FALSE, recursive = FALSE)
  sorted_files_full <- list.dirs(file.path(sort_dir, "sorted_working"), full.names = TRUE, recursive = FALSE)
  
  # Get possible loggers from the sorting folders
  possible_loggers <- unique(sapply(strsplit(sorted_files, "_"), "[[", 1))
  possible_tables <- unique(sapply(strsplit(sorted_files, "_"), "[[", 2))
  
  # Get logger and table from input
  logger <- tolower(logger)
  table <- tolower(table)
  
  # Set a bunch of possible error messages if you didn't enter a possible logger/table combination
...
##  ======================= Function 8: norm_gcrew ===========================================
# FUNCTION 8: norm_gcrew(source_dir, norm_dir, table_type, design_table, plot_names, increment)
...
norm_gcrew <- function(source_dir, norm_dir, design_table, plot_names, increment){
  ...
  for(n in 1:length(i)){
    ...
    # Format time
    # If time format is correct, format the time to POSIXct
    # If not, print the time
    if(length(which(!grepl("20..-..-.. ..:..:..", dt$timestamp))) == 0){
      Sys.setenv(TZ = "America/Jamaica") ### set for EST all year long
      dt$timestamp <- as.POSIXct(dt$timestamp, format = "%Y-%m-%d %H:%M:%S", tz = "America/Jamaica")
    } else {
      print(paste0("These times are formatted incorrectly: ", dt$timestamp[which(!grepl("20..-..-.. ..:..:..", dt$timestamp))]))
    }
    ...
    if(("cr1000_name" %in% colnames(dt_plot2)) == F){
      ...
      # Change cast to using research_name and using aggregation function (unique) in case of duplicates
      dt_site_wide <- dcast(dt_site_merged, site+logger+rowid+time2+timestamp ~ research_name, fun.aggregate = function(x){unique(x)[1]},
                            subset = NULL, drop = TRUE, value.var = "value") # note that fill should be allowed to default
      ...
      # New section: prep site data to join with design data so we can get the research name
      # Prep site data
      dt_site2 <- unique(dt_site2)
      # Merge site data with design so that we can get the research name
      dt_site3 <- merge(dt_site2, design, allow.cartesian = TRUE, by = designkey)
      
      # Select only the variables we want
      dt_site4 <- subset(dt_site3, select = c("site", "logger", "rowid", "time2", "timestamp",
                                              "value", "research_name", "cr1000_name", "type"))
      ...
      #                       subset = NULL, drop = TRUE, value.var = "value") # note that fill should be allowed to default
      dt_site_wide <- dcast(dt_site4, rowid+logger+time2+timestamp ~ research_name, fun.aggregate = function(x){unique(x)[1]},
                            subset = NULL, drop = TRUE, value.var = "value") # note that fill should be allowed to default
    ...
    dt_normalized <- dt_normalized[order(dt_normalized$time2),]
    ...
    # Format times
    dt_normalized$timestamp <- format(as.POSIXct(dt_normalized$timestamp, tz="America/Jamaica"), 
                                      format="%Y-%m-%d %H:%M:%S")
    
    dt_normalized$time2 <- format(as.POSIXct(dt_normalized$time2, tz="America/Jamaica"), 
                                      format="%Y-%m-%d %H:%M:%S")
    ...
    file.copy(i[n], copy_dir, overwrite = T)
...
monthly_files <- function(norm_dir, table) {
  ...
  norm_files <- list.files(file.path(norm_dir, "normal_working"), pattern = NULL, all.files = FALSE,
                           full.names = FALSE, recursive = FALSE,
                           ignore.case = FALSE)
  norm_files_full <- list.files(file.path(norm_dir, "normal_working"), pattern = NULL, all.files = FALSE,
                                full.names = TRUE, recursive = FALSE,
                                ignore.case = FALSE)
  
  # Create list of possible tables 
  possible_tables <- tolower(unique(sapply(strsplit(norm_files, "_"), "[[", 2)))
  
  # Get table that was user entered
  table <- tolower(table)
  
  # Enter error messages if search message is not working
...
monthlymanage <- function(source_files, monthly_dir, out_path, logger_role){
  # Get working directories
  norm_working <- file.path(unlist(strsplit(source_files[1], "/normal_working/"))[1], "normal_working")
  norm_archive <- file.path(unlist(strsplit(source_files[1], "/normal_working/"))[1], "normal_archive")
  # Read in logger/folder pairing file
  ...
  for (n in 1:length(source_files)){
    # Read in data table 
    dt <- fread(source_files[n], tz = "")
    
    # Format time
    if(length(which(!grepl("20..-..-.. ..:..:..", dt$time2))) == 0){
      Sys.setenv(TZ = "America/Jamaica") ### set for EST all year long
      dt$time2 <- as.POSIXct(dt$time2, format = "%Y-%m-%d %H:%M:%S", tz = "America/Jamaica")
      dt$timestamp <- as.POSIXct(dt$timestamp, format = "%Y-%m-%d %H:%M:%S", tz = "America/Jamaica")
    } else {
    ...
    if(grepl("waterlevel", source_files[n], ignore.case = TRUE) == TRUE){
      folder <- "gcrew_waterlevel"
    } else{
      folder <- logger_role$project[which(logger_role$pattern == logger)]
    }
    
    output_dir <- file.path(out_path, folder, "monthly")
    ...
    for (m in 1:nrow(y2)) {
      ...
      if (file.exists(path_out) == TRUE) {
        # Read data table that exists
        dt2 <- fread(path_out, tz = "")
        
        if(length(which(!grepl("20..-..-.. ..:..:..", dt2$time2))) == 0){
          Sys.setenv(TZ = "America/Jamaica") ### set for EST all year long
          dt2$time2 <- as.POSIXct(dt2$time2, format = "%Y-%m-%d %H:%M:%S", tz = "America/Jamaica")
          dt2$timestamp <- as.POSIXct(dt2$timestamp, format = "%Y-%m-%d %H:%M:%S", tz = "America/Jamaica")
        } else {
        ...
        # Sort by time
        dt2 <- arrange(dt2, time2)
        
        dt2$time2[1]
        
        # Format time for output
        dt2$time2 <- format(as.POSIXct(dt2$time2, tz="America/Jamaica"), 
                               format="%Y-%m-%d %H:%M:%S")
        
        dt2$timestamp <- format(as.POSIXct(dt2$timestamp, tz="America/Jamaica"), 
                            format="%Y-%m-%d %H:%M:%S")
        dt2$time2[1]
        class(dt2$time2)
        ...
        # Format time for output
        dtplot$time2 <- format(as.POSIXct(dtplot$time2, tz="America/Jamaica"), 
                                      format="%Y-%m-%d %H:%M:%S")
        dtplot$timestamp <- format(as.POSIXct(dtplot$timestamp, tz="America/Jamaica"), 
                               format="%Y-%m-%d %H:%M:%S")
        dtplot$time2[1]
        class(dtplot$time2)
    ...
    file.copy(source_files[n], norm_archive, overwrite = T)
...
bundle <- function(monthly_dir, out_path, logger_role, logger, table){
  # Read in pairing document for logger/table x project folder
  logger_role <- read.csv(logger_role)
  
  # Create list of file paths in source directory
  i <- list.files(monthly_dir, pattern = paste0(logger, "_", table), all.files = FALSE,
                  full.names = TRUE, recursive = TRUE,
                  ignore.case = FALSE)
  ...
  # create a logger / table / year data frame for all combinations
  ...
  for(n in 1:nrow(lt_id)){
    ...
    if (file.exists(newpath) == FALSE){
      # Format time for output
      dt$time2 <- format(as.POSIXct(dt$time2, tz="America/Jamaica"), 
                          format="%Y-%m-%d %H:%M:%S")
      
      dt$timestamp <- format(as.POSIXct(dt$timestamp, tz="America/Jamaica"), 
                              format="%Y-%m-%d %H:%M:%S")
...
# These are other functions that are only run as needed

## ========================== Function 12: headercheck =================================================
