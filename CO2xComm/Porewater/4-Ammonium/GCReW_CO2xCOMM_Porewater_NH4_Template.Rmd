---
title: "CO2xComm: Porewater Ammonium"
...
#identify the files you want to read in 
  ...
  NH3_files <- c("Raw Data/LTREB_1M_July_2025.csv")
...
# Define the file path for QAQC log file - NO Need to change just check year 
  ...
  final_path <- "Processed Data/GCReW_CO2xCOMM_Porewater_NH4_202507.csv"
...
samples <- df_all %>% 
  filter(str_detect(Sample_Name, c("SP")) | str_detect(Sample_Name, c("SC"))) %>% 
...
org_labeled_fin_data <- final_data_labeled %>%
  rename(
    NH4_Conc_uM = Conc,
   NH4_Conc_Flag = Conc_flag,
    NH4_QAQC_Flag = QAQC_Flag
  )
...
#need to work out how best to plot these data 
 ...
 NH4_forplot <- NH4_forplot[order(NH4_forplot$CO2_Treatment), ]
 
viz_NH4_plot2 <- ggplot(data = NH4_forplot, aes(
...
NH4_forplot <- NH4_forplot[order(NH4_forplot$Depth_cm), ]
...
#make plot
viz_NH4_plot_2 <- ggplot(data = NH4_forplot, aes(x = NH4_Conc_uM, y = as.numeric(Depth_cm), color = Plot_Color)) +
  geom_point(size = 3) +
  geom_path(linewidth = 1) +
  scale_color_manual(
    values = c(
  ...
  labs(
    x = "NH4 (uM)",
    y = "Depth (cm)",
    title = "CO2xCOMM: Porewater NH4 by Replicate",
    color = "Replicate"
  ) +
