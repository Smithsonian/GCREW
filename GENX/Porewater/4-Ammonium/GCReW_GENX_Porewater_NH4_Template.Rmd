...
Slope_raw_list <- map(
 ...
 Slope_Std_Crvs <- Slope_raw %>% 
   ...
   distinct() %>% 
   mutate(R2_Flag = ifelse(R2 >= r2_cutoff, "R2_Pass - PROCEED", "R2_Fail - REASSESS"))
...
add_log <- R2_Check %>% dplyr::select(Test, Run_Date, R2, Slope, Intercept) %>%
  mutate(Test = case_when(
    Test == "Ammonia 2" ~ "NH3"
  )) %>% mutate(Run_Date = as.Date(as.character(Run_Date), format = "%m/%d/%Y")) %>% rename(User = user, R_Squared = R2) %>% mutate(Run = "Current Runs")#Format new run data for log
...
samples <- df_all %>% 
  filter(grepl(pattern = "^Ch\\d{1,2}-\\d{1,2}$", Sample_Name)) %>% filter(!str_detect(Sample_Name, c("mg"))) %>% 
...
#Pull out samples 
samples <- df_all_cor %>% filter(grepl(pattern = "^Ch\\d{1,2}-\\d{1,2}$", Sample_Name)) 
...
Flags_Bad <- Flags_joined %>% mutate(across(!1:2, \(x) ifelse(x %ilike% "PROCEED" | x %ilike% "NA" | is.na(x), "", x))) %>% distinct()
...
### Ammonia 
NH4_forplot <- df_all_clean

NH4_forplot$Depth_cm <- 
  as.numeric(NH4_forplot$Depth_cm)

#group the data for plotting
NH4_forplot <- NH4_forplot %>%
  group_by(Depth_cm) %>%
...
#create plot
viz_NH4_plot <- ggplot(data = NH4_forplot, aes(
  x = as.factor(Chamber),
  y = Conc,
...
#make plot
viz_NH4_plot_2 <- ggplot(data = NH4_forplot, 
                            aes(x = Conc, y = Depth_cm, color = Chamber)) +
  geom_point(size = 3) +
  geom_path(linewidth = 1) +
  ...
  labs(x = "NH4", 
       y = "Depth (cm)", 
       title = "GENX: Porewater NH4 by Chamber") +
