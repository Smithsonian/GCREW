...
#record any notes about the run or anything other info here: 
  run_notes <- "20260608 run stopped part way through; ch1-80 not collected"
...
add_log <- R2_Check %>% dplyr::select(Test, Run_Date, R2, Slope, Intercept) %>%
  mutate(Test = case_when(
    Test == "Ammonia 2" ~ "NH3"
  )) %>% mutate(Run_Date = as.Date(as.character(Run_Date), format = "%m/%d/%Y")) %>% mutate(User = user) %>% mutate(Run = "Current Runs") %>%
  rename(R_Squared = R2)#Format new run data for log


new_log <- rbind(old_log, add_log) %>% mutate(across(where(is.numeric), \(x) round(x, digits = 4))) %>% distinct(dplyr::select(., -User), .keep_all = TRUE) #bind rows, round data, and remove possible duplicate data from rerunning code
