...
###### Run information - PLEASE CHANGE
  Date_Run = "2026-09-02"  #Date that instrument was run
  Run_by = "Victoria Ellis"  #Instrument user 
  Script_run_by = "Victoria Ellis" #Code user 
  sample_year <- "2026"
  sample_month <- "May"
  Site = "GCREW"
  run_notes = "CH2-80 Cl is very high (321mM) with a bad spike, 
        likely human error, will re-run with GENX July run" #any notes from the run

###### File Names - PLEASE CHANGE 
#file path and name for raw summary data file 
  raw_file_name_cl = "Raw Data/GENX_202605_Cl.txt"
  raw_file_name_so4 = "Raw Data/GENX_202605_SO4.txt"

#file path and name of processed data file 
  processed_file_name = "Processed Data/GCReW_GENX_PW_Processed_Cl_SO4_202605.csv" 
...
dionex_metadata <- dionex_metadata %>%
  mutate(sample_ID = paste0(Sample_ID, "_", Year))

#check to see if all samples are present in the metadata 
all_present <- all(all_data_flagged$sample_ID %in% dionex_metadata$sample_ID)
...
df_all_clean <- all_data_flagged %>%
  left_join(dionex_metadata, by = "sample_ID")

```


## Visualize Data - Cl
```{r Visualize Data - Cl, fig.height=10, echo=FALSE}
cat("Visualize Data - Cl")

Cl_forplot <- df_all_clean

Cl_forplot$Depth_cm <- 
  as.numeric(Cl_forplot$Depth_cm)
...
#create plot
viz_Cl_plot <- ggplot(data = Cl_forplot, aes(
  x = as.factor(Chamber),
  y = Cl_Conc_mM,
...
\newpage
```{r Visualize Data by Rep - Cl,  fig.height=8.5, echo=FALSE}
...
#make plot
viz_Cl_plot_2 <- ggplot(data = Cl_forplot, 
                            aes(x = Cl_Conc_mM, y = Depth_cm, color = Chamber)) +
  geom_point(size = 3) +
  ...
  labs(x = "Cl Conc (mM)", 
       y = "Depth (cm)", 
       title = "GENX: Porewater Cl by Chamber") +
...
## Visualize Data - SO4
```{r Visualize Data - SO4, fig.height=10, echo=FALSE}
cat("Visualize Data - SO4")

### Ammonia 
SO4_forplot <- df_all_clean

SO4_forplot$Depth_cm <- 
  as.numeric(SO4_forplot$Depth_cm)
...
#create plot
viz_SO4_plot <- ggplot(data = SO4_forplot, aes(
  x = as.factor(Chamber),
  y = SO4_Conc_mM,
  fill = as.factor(Chamber)
)) +
...
\newpage
```{r Visualize Data by Rep - SO4,  fig.height=8.5, echo=FALSE}
...
#make plot
viz_SO4_plot_2 <- ggplot(data = SO4_forplot, 
                            aes(x = SO4_Conc_mM, y = Depth_cm, color = Chamber)) +
  geom_point(size = 3) +
  geom_path(linewidth = 1) +
  scale_color_manual(breaks = c(
       "Ch1", "Ch2", "Ch3", "Ch4", "Ch5", "Ch6", "Ch7", "Ch8", "Ch9", "Ch10", "Ch11", "Ch12"),
  ...
  labs(x = "SO4 Conc (mM)", 
       y = "Depth (cm)", 
       title = "GENX: Porewater SO4 by Chamber") +
  scale_y_reverse(breaks = c(5, 20, 40, 80),
                  limits = c(85, 0)) +
  xlim(0, 35) +
  theme_classic() +
