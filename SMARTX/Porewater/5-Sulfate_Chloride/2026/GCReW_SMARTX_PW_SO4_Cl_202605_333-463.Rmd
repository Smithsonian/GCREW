---
title: "SMARTX: Porewater SO4/Cl"
author: "May 2026 Samples 333-463"
date: "`r Sys.Date()`"
output: 
  pdf_document:
    toc: true
...
###### Run information - PLEASE CHANGE
  Date_Run = "2026-08-17"  #Date that instrument was run
  Run_by = "Victoria Ellis"  #Instrument user 
  Script_run_by = "Victoria Ellis" #Code user 
  sample_year <- "2026"
  sample_month <- "May"
  Site = "GCREW"
  run_notes = "One check standard is out of range" #any notes from the run

###### File Names - PLEASE CHANGE 
#file path and name for raw summary data file 
  raw_file_name_cl = "Raw Data/SMARTX_202605_333-463_Cl.txt"
  raw_file_name_so4 = "Raw Data/SMARTX_202605_333-463_SO4.txt"

#file path and name of processed data file 
  processed_file_name = "Processed Data/GCReW_SMARTX_PW_Processed_Cl_SO4_202605_333-463.csv" 
...
#downloaded metadata csv - downloaded from Google drive as csv for this year
  ...
  Log_path = here("Porewater QAQC Logs", "GCReW_SO4_Cl_QAQC_Log.csv")
...
dionex_metadata <- raw_metadata %>%
  filter(grepl("SMARTX", Project, ignore.case = TRUE)) %>%
  mutate(Sample.ID = gsub("-", "_", Sample.ID)) %>% 
  mutate(Cl_SO4_ID = paste0(Sample.ID, "_", sample_month, "_", sample_year)) %>%
...
QAspks$SampleVol <- 1
QAspks$SampleVol <-  ifelse(QAspks$Dilution == 99.6667, 1495, QAspks$SampleVol)
...
QAspks$SO4_Total_spkd <- (QAspks$SO4_Conc_mM_spk/100.3333)*(QAspks$SampleVol+spkvol) 
...
#change sample IDs to match metdata
all_data_flagged <- all_data_flagged %>%
  mutate(sample_ID = paste0(sample_ID, "_", sample_year))
...
```{r Visualize Data C3 Cl, fig.height=10, echo=FALSE}
cat("Visualize Data - Cl")
Cl_forplot <- df_all_clean 

#C3 plots
Cl_forplot_C3 <- Cl_forplot %>%
  filter(Nickname == "C3")

Cl_forplot_C3$Depth_cm <- 
  as.numeric(Cl_forplot_C3$Depth_cm)

#group the data for plotting
Cl_forplot_C3 <- Cl_forplot_C3 %>%
  group_by(Depth_cm) %>%
...
#create plot
viz_Cl_plot_C3 <- ggplot(data = Cl_forplot_C3, aes(
  ...
  coord_flip()
print(viz_Cl_plot_C3)

```
\newpage
```{r Visualize Data C4 Cl, fig.height=10, echo=FALSE}
#C4 plots

Cl_forplot <- df_all_clean

Cl_forplot_C4 <- Cl_forplot %>%
  filter(Nickname == "C4")

Cl_forplot_C4$Depth_cm <-
  as.numeric(Cl_forplot_C4$Depth_cm)

#group the data for plotting
Cl_forplot_C4 <- Cl_forplot_C4 %>%
  group_by(Depth_cm) %>%
  mutate(row_num = factor(row_number())) %>%  # create row_num as factor per group
  ungroup()

viz_Cl_plot_C4 <- ggplot(data = Cl_forplot_C4, aes(
  x = as.factor(Temp_Treatment),
  y = Cl_Conc_mM,
  ...
  coord_flip()
print(viz_Cl_plot_C4)

```
\newpage
```{r Visualize Data by Rep C3 Cl,  fig.height=8.5, echo=FALSE}
...
Cl_forplot_C3 <- Cl_forplot_C3[order(Cl_forplot_C3$Depth_cm), ]

#make plot
viz_Cl_plot_C3_2 <- ggplot(data = Cl_forplot_C3, 
                            aes(x = Cl_Conc_mM, y = Depth_cm, color = Replicate)) +
  geom_point(size = 3) +
  ...
  labs(x = "Cl (mM)", 
       y = "Depth (cm)", 
       title = "C3: Porewater Cl by Replicate") +
...
```{r Visualize Data by Rep C4 Cl,  fig.height=5.7, echo=FALSE}
...
Cl_forplot_C4 <- Cl_forplot_C4[order(Cl_forplot_C4$Depth_cm), ]

#make plot
viz_Cl_plot_C4_2 <- ggplot(data = Cl_forplot_C4,
                            aes(x = Cl_Conc_mM, y = Depth_cm, color = Replicate)) +
  geom_point(size = 3) +
  geom_path(linewidth = 1) +
  scale_color_manual(values = c(
  ...
  labs(x = "Cl (mM)",
       y = "Depth (cm)",
       title = "C4: Porewater Cl by Replicate") +
  scale_y_reverse(breaks = c(10, 20, 40, 80, 120),
                  limits = c(160, 5)) +
  xlim(125, 175) +
  theme_classic() +
...
```{r Visualize Data C3 SO4, fig.height=10, echo=FALSE}
cat("Visualize Data - SO4")

### Ammonia 
SO4_forplot <- df_all_clean

#C3 plots
SO4_forplot_C3 <- SO4_forplot %>%
  filter(Nickname == "C3")

SO4_forplot_C3$Depth_cm <- 
  as.numeric(SO4_forplot_C3$Depth_cm)

#group the data for plotting
SO4_forplot_C3 <- SO4_forplot_C3 %>%
  group_by(Depth_cm) %>%
  mutate(row_num = factor(row_number())) %>%  # create row_num as factor per group
  ungroup() %>%
  mutate(Treatment = ifelse(Temp_Treatment == 0, ifelse(CO2_Treatment == "Ambient", "0 C + aCO2", "0 C + eCO2"), ifelse(Temp_Treatment == 1.7, "1.7 C + aCO2", ifelse(Temp_Treatment == 3.4, "3.4 C + aCO2", ifelse(Temp_Treatment == 5.1, ifelse(CO2_Treatment == "Ambient", "5.1 C + aCO2", "5.1 C + eCO2"), NA)))))
...
#create plot
viz_SO4_plot_C3 <- ggplot(data = SO4_forplot_C3, aes(
  x = as.factor(Temp_Treatment),
  y = SO4_Conc_mM,
  fill = as.factor(Treatment),
  color = as.factor(Treatment)
  ...
  coord_flip()
print(viz_SO4_plot_C3)

```
\newpage
```{r Visualize Data C4 SO4, fig.height=10, echo=FALSE}
#C4 plots
SO4_forplot_C4 <- SO4_forplot %>%
  filter(Nickname == "C4")

SO4_forplot_C4$Depth_cm <-
  as.numeric(SO4_forplot_C4$Depth_cm)

#group the data for plotting
SO4_forplot_C4 <- SO4_forplot_C4 %>%
  group_by(Depth_cm) %>%
  mutate(row_num = factor(row_number())) %>%  # create row_num as factor per group
  ungroup()

viz_SO4_plot_C4 <- ggplot(data = SO4_forplot_C4, aes(
  x = as.factor(Temp_Treatment),
  y = SO4_Conc_mM,
  fill = as.factor(Temp_Treatment)
)) +
  ...
  coord_flip()
print(viz_SO4_plot_C4)

```
\newpage
```{r Visualize Data by Rep C3 SO4,  fig.height=8.5, echo=FALSE}
...
#ensure points are in correct order
SO4_forplot_C3 <- SO4_forplot_C3[order(SO4_forplot_C3$Depth_cm), ]

#make plot
viz_SO4_plot_C3_2 <- ggplot(data = SO4_forplot_C3, 
                            aes(x = SO4_Conc_mM, y = Depth_cm, color = Replicate)) +
  geom_point(size = 3) +
  geom_path(linewidth = 1) +
  scale_color_manual(values = c(
    "1" = "#FFD500",
  ...
  labs(x = "SO4 (mM)", 
       y = "Depth (cm)", 
       title = "C3: Porewater SO4 by Replicate") +
  scale_y_reverse(breaks = c(10, 20, 40, 80, 120),
                  limits = c(125, 5)) +
  #xlim(0, 12) +
  theme_classic() +
...
```{r Visualize Data by Rep C4 SO4,  fig.height=5.7, echo=FALSE}
...
#make sure point are in correct order
SO4_forplot_C4 <- SO4_forplot_C4[order(SO4_forplot_C4$Depth_cm), ]

#make plot
viz_SO4_plot_C4_2 <- ggplot(data = SO4_forplot_C4,
                            aes(x = SO4_Conc_mM, y = Depth_cm, color = Replicate)) +
  geom_point(size = 3) +
  geom_path(linewidth = 1) +
  scale_color_manual(values = c(
  ...
  labs(x = "SO4 (mM)",
       y = "Depth (cm)",
       title = "C4: Porewater SO4 by Replicate") +
  scale_y_reverse(breaks = c(10, 20, 40, 80, 120),
                  limits = c(125, 5)) +
  xlim(0, 25) +
  theme_classic() +
