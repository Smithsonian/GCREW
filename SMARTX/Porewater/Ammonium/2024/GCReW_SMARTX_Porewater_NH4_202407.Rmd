...
#identify the files you want to read in 
  ...
  NH3_files <- c("Raw Data/July_2024_SMARTX_NH4_Run1.csv",
                 #"Raw Data/July_2024_SMARTX_NH4_Run2.csv", #this is the same file as run1, downloaded wrong?
                 "Raw Data/July_2024_SMARTX_NH4_Run3.csv",
                 "Raw Data/July_2024_SMARTX_NH4_Run4.csv")

# Define the file path for QAQC log file - NO Need to change just check year 
  file_path <- "Raw Data/SEAL_SMARTX_QAQC_Log_2024.csv"
  final_path <- "Processed Data/GCReW_SMARTX_Porewater_NH4_202407.csv"

#record any notes about the run or anything other info here: 
  run_notes <- "Run 1 and Run 2 files are the exact same, looks like wrong file downloaded."
...
metadata <- raw_metadata %>%
  filter(grepl("SMARTX", Experiment, ignore.case = TRUE)) %>%
  mutate(Sample_Name = paste0(Experiment, "_", Sample.ID)) %>%
...
metadata <- metadata %>%
  mutate(
    Community = case_when(
      startsWith(as.character(Plot_Name), "3") ~ "C3",
      startsWith(as.character(Plot_Name), "6") ~ "C3",
      startsWith(as.character(Plot_Name), "4") ~ "C4",
...
NH3_peChk <- df_all_cor %>%
  filter(Test == "Ammonia 2", 
         str_detect(Sample_Name, "peCheck")) %>%
  summarise(
...
viz_NH3_plot_C3 <- ggplot(data = NH3_forplot_C3, aes(x = as.factor(Temp_Treatment), y = Conc, fill=as.factor(Temp_Treatment))) +
...
#would be good to come up with a way to parse out and show the elevated CO2 as well
...
df_all_clean_cols <- df_all_clean %>%
  select(Sample_Name, Sample_ID, Experiment, Plot_Name, Depth, CO2_Treatment, Temp_Treatment, Community,
         Year, Month,
...
final_data <- final_data_labeled %>%
  ...
  select(Site, Experiment, Month, Year, Sample_Name, Sample_ID,  
         Plot_Name, Depth, CO2_Treatment, Temp_Treatment, Community,
