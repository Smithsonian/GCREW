...
#Order by Temperature Treatment
data_plotting$Temp_Treatment = factor(data_plotting$Temp_Treatment, levels = c("0",  "1.7", "3.4", "5.1"))
data_plotting <- data_plotting[order(data_plotting$Temp_Treatment), ]

#Order by Depth
data_plotting$Depth_cm = factor(data_plotting$Depth_cm, levels = c("10",  "20", "40", "80", "120"))
...
viz_cl_plot <- ggplot(data_plotting, aes(x = as.factor(Temp_Treatment), y = Cl_ppm, fill = as.factor(Temp_Treatment))) +
  ...
  theme_classic() +
  labs(x = " ", y = "Cl (mg/L)", title = "Samples: Chloride", fill="+Temp. (C)") +
...
viz_so4_plot <-  ggplot(data_plotting, aes(x = as.factor(Temp_Treatment), y = SO4_ppm, fill = as.factor(Temp_Treatment))) +
  geom_bar(stat = "identity", position = position_dodge2(preserve = "single"), color="black") +   
  facet_grid(~ Depth_cm, labeller = labeller(Depth_cm = depth_labels), scales="free_x") +
  scale_fill_manual(values = c(
    "0" = "blue", 
  ...
  theme_classic() +
  labs(x = " ", y = "SO4 (mg/L)", title = "Samples: SO4", fill="+Temp. (C)") +
  scale_x_discrete(drop = TRUE) + 
  theme(legend.position = "right") +
  theme(axis.text.x = element_text(angle = 0, hjust = 0.5), 
  panel.border = element_rect(color = "black", fill = NA, linewidth = 0.8))
