...
cat("Run Information: CODE USER ") #lets you know what section you're in 
#set the run date & user name 
  run_date <- "09/19/2024"
  sample_year <- "YYYY"
  sample_month <- "Month"
  user <- "Instrument User Name"
...
#record any notes about the run or anything other info here: 
  run_notes <- "Put any notes about the run "
...
## Matrix Effects - seems like wasn't run on this one? 
...
df_all_clean_cols <- df_all_clean %>%
  select(Sample_Name, Sample_ID, Experiment, Plot_Name, Depth, CO2_Treatment, Temp_Treatment, Plant_Type,
...
final_data <- final_data_labeled %>%
  ...
  select(Site, Experiment, Sample_Name, Sample_ID,  
         Plot_Name, Depth, CO2_Treatment, Temp_Treatment, Plant_Type,
